...
#include "qgeoroutereplyKokudo.h"
...
const QString osrmInstructionText(const QString &instructionCode, const QString &wayname)
{
    if (instructionCode == QLatin1String("0")) {
        ...
        if (wayname.isEmpty())
            return QGeoRouteReplyKokudo::tr("Go straight.");
        else
            return QGeoRouteReplyKokudo::tr("Go straight onto %1.").arg(wayname);
    } else if (instructionCode == QLatin1String("2")) {
        if (wayname.isEmpty())
            return QGeoRouteReplyKokudo::tr("Turn slightly right.");
        else
            return QGeoRouteReplyKokudo::tr("Turn slightly right onto %1.").arg(wayname);
    } else if (instructionCode == QLatin1String("3")) {
        if (wayname.isEmpty())
            return QGeoRouteReplyKokudo::tr("Turn right.");
        else
            return QGeoRouteReplyKokudo::tr("Turn right onto %1.").arg(wayname);
    } else if (instructionCode == QLatin1String("4")) {
        if (wayname.isEmpty())
            return QGeoRouteReplyKokudo::tr("Make a sharp right.");
        else
            return QGeoRouteReplyKokudo::tr("Make a sharp right onto %1.").arg(wayname);
    }
    else if (instructionCode == QLatin1String("5")) {
        return QGeoRouteReplyKokudo::tr("When it is safe to do so, perform a U-turn.");
    } else if (instructionCode == QLatin1String("6")) {
        if (wayname.isEmpty())
            return QGeoRouteReplyKokudo::tr("Make a sharp left.");
        else
            return QGeoRouteReplyKokudo::tr("Make a sharp left onto %1.").arg(wayname);
    } else if (instructionCode == QLatin1String("7")) {
        if (wayname.isEmpty())
            return QGeoRouteReplyKokudo::tr("Turn left.");
        else
            return QGeoRouteReplyKokudo::tr("Turn left onto %1.").arg(wayname);
    } else if (instructionCode == QLatin1String("8")) {
        if (wayname.isEmpty())
            return QGeoRouteReplyKokudo::tr("Turn slightly left.");
        else
            return QGeoRouteReplyKokudo::tr("Turn slightly left onto %1.").arg(wayname);
    } else if (instructionCode == QLatin1String("9")) {
        return QGeoRouteReplyKokudo::tr("Reached waypoint.");
    } else if (instructionCode == QLatin1String("10")) {
        if (wayname.isEmpty())
            return QGeoRouteReplyKokudo::tr("Head on.");
        else
            return QGeoRouteReplyKokudo::tr("Head onto %1.").arg(wayname);
    } else if (instructionCode == QLatin1String("11")) {
        return QGeoRouteReplyKokudo::tr("Enter the roundabout.");
    } else if (instructionCode == QLatin1String("11-1")) {
        if (wayname.isEmpty())
            return QGeoRouteReplyKokudo::tr("At the roundabout take the first exit.");
        else
            return QGeoRouteReplyKokudo::tr("At the roundabout take the first exit onto %1.").arg(wayname);
    } else if (instructionCode == QLatin1String("11-2")) {
        if (wayname.isEmpty())
            return QGeoRouteReplyKokudo::tr("At the roundabout take the second exit.");
        else
            return QGeoRouteReplyKokudo::tr("At the roundabout take the second exit onto %1.").arg(wayname);
    } else if (instructionCode == QLatin1String("11-3")) {
        if (wayname.isEmpty())
            return QGeoRouteReplyKokudo::tr("At the roundabout take the third exit.");
        else
            return QGeoRouteReplyKokudo::tr("At the roundabout take the third exit onto %1.").arg(wayname);
    } else if (instructionCode == QLatin1String("11-4")) {
        if (wayname.isEmpty())
            return QGeoRouteReplyKokudo::tr("At the roundabout take the fourth exit.");
        else
            return QGeoRouteReplyKokudo::tr("At the roundabout take the fourth exit onto %1.").arg(wayname);
    } else if (instructionCode == QLatin1String("11-5")) {
        if (wayname.isEmpty())
            return QGeoRouteReplyKokudo::tr("At the roundabout take the fifth exit.");
        else
            return QGeoRouteReplyKokudo::tr("At the roundabout take the fifth exit onto %1.").arg(wayname);
    } else if (instructionCode == QLatin1String("11-6")) {
        if (wayname.isEmpty())
            return QGeoRouteReplyKokudo::tr("At the roundabout take the sixth exit.");
        else
            return QGeoRouteReplyKokudo::tr("At the roundabout take the sixth exit onto %1.").arg(wayname);
    } else if (instructionCode == QLatin1String("11-7")) {
        if (wayname.isEmpty())
            return QGeoRouteReplyKokudo::tr("At the roundabout take the seventh exit.");
        else
            return QGeoRouteReplyKokudo::tr("At the roundabout take the seventh exit onto %1.").arg(wayname);
    } else if (instructionCode == QLatin1String("11-8")) {
        if (wayname.isEmpty())
            return QGeoRouteReplyKokudo::tr("At the roundabout take the eighth exit.");
        else
            return QGeoRouteReplyKokudo::tr("At the roundabout take the eighth exit onto %1.").arg(wayname);
    } else if (instructionCode == QLatin1String("11-9")) {
        if (wayname.isEmpty())
            return QGeoRouteReplyKokudo::tr("At the roundabout take the ninth exit.");
        else
            return QGeoRouteReplyKokudo::tr("At the roundabout take the ninth exit onto %1.").arg(wayname);
    } else if (instructionCode == QLatin1String("12")) {
        if (wayname.isEmpty())
            return QGeoRouteReplyKokudo::tr("Leave the roundabout.");
        else
            return QGeoRouteReplyKokudo::tr("Leave the roundabout onto %1.").arg(wayname);
    } else if (instructionCode == QLatin1String("13")) {
        return QGeoRouteReplyKokudo::tr("Stay on the roundabout.");
    } else if (instructionCode == QLatin1String("14")) {
        if (wayname.isEmpty())
            return QGeoRouteReplyKokudo::tr("Start at the end of the street.");
        else
            return QGeoRouteReplyKokudo::tr("Start at the end of %1.").arg(wayname);
    } else if (instructionCode == QLatin1String("15")) {
        return QGeoRouteReplyKokudo::tr("You have reached your destination.");
    } else {
        return QGeoRouteReplyKokudo::tr("Don't know what to say for '%1'").arg(instructionCode);
    }
}

QGeoRouteReplyKokudo::QGeoRouteReplyKokudo(QNetworkReply *reply, const QGeoRouteRequest &request,
                                     QObject *parent)
...
QGeoRouteReplyKokudo::~QGeoRouteReplyKokudo()
{
    if (m_reply)
        m_reply->deleteLater();
}

void QGeoRouteReplyKokudo::abort()
{
    if (!m_reply)
        return;
...
void QGeoRouteReplyKokudo::networkReplyFinished()
{
    if (!m_reply)
        return;
...
void QGeoRouteReplyKokudo::networkReplyError(QNetworkReply::NetworkError error)
{
    Q_UNUSED(error)
